	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-meta commentmetadata"><a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
			<?php
				...
				printf( esc_html__( '%1$s at %2$s', 'live-it-up' ), get_comment_date(),  get_comment_time() ); ?></a><?php edit_comment_link( esc_html__( '(Edit)','live-it-up' ), ' ' );
			?>
...
function live_it_up_posted_on() {
	printf( __( '<span class="%1$s">Posted on</span> %2$s <span class="meta-sep">by</span> %3$s', 'live-it-up' ),
...
function live_it_up_posted_in() {
	...
	printf(
		$posted_in,
		get_the_category_list( esc_html__( ', ', 'live-it-up' ) ),
		$tag_list,
		esc_url( get_permalink() ),
